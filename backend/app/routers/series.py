...
def _series_to_response(series: Series, book_count: int = 0) -> SeriesResponse:
...
async def list_series(db: AsyncSession = Depends(get_db)):
    ...
    return [_series_to_response(s, count) for s, count in result.all()]
...
async def get_series(series_id: str, db: AsyncSession = Depends(get_db)):
    ...
    s, count = row
    return _series_to_response(s, count)
